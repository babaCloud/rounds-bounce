        ...
        for (int i = 0; i < verts.Count; i++)
        {
            if (i % 4 == 0)
            {
                ...
                if (useRotation)
                {
                    ...
                    {
                        case RotateAxis.x:
                            uiVertex0.position = Quaternion.AngleAxis(offset, Vector3.right) * (uiVertex0.position - center) + center;
                            uiVertex1.position = Quaternion.AngleAxis(offset, Vector3.right) * (uiVertex1.position - center) + center;
                            uiVertex2.position = Quaternion.AngleAxis(offset, Vector3.right) * (uiVertex2.position - center) + center;
                            uiVertex3.position = Quaternion.AngleAxis(offset, Vector3.right) * (uiVertex3.position - center) + center;
                            break;
                        case RotateAxis.y:
                            uiVertex0.position = Quaternion.AngleAxis(offset, Vector3.up) * (uiVertex0.position - center) + center;
                            uiVertex1.position = Quaternion.AngleAxis(offset, Vector3.up) * (uiVertex1.position - center) + center;
                            uiVertex2.position = Quaternion.AngleAxis(offset, Vector3.up) * (uiVertex2.position - center) + center;
                            uiVertex3.position = Quaternion.AngleAxis(offset, Vector3.up) * (uiVertex3.position - center) + center;
                            break;
                        case RotateAxis.z:
